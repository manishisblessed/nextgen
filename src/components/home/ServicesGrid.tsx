          ...
          {services.map((s) => {
            ...
            return (
              <StaggerItem key={s.slug}>
                <Link
                  href={s.href}
                  className="group relative block h-full overflow-hidden rounded-2xl border border-ink-100 bg-white p-5 transition-all duration-300 ease-out hover:-translate-y-1.5 hover:border-brand-200 hover:shadow-soft"
                >
                  <div
                    className={cn(
                  ...
                  <div className="flex items-start justify-between">
                    <span
                      className={cn(
                        "grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-3",
                        categoryStyles[s.category]
                      )}
                    >
                  ...
                  <h3 className="mt-4 font-display text-base font-semibold text-ink-900 transition-colors duration-300 group-hover:text-brand-700">
                    {s.title}
                  </h3>
                  <p className="mt-1 text-sm text-ink-500">{s.description}</p>
                  <span className="mt-4 inline-flex translate-x-[-4px] items-center gap-1 text-xs font-semibold text-brand-700 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100">
