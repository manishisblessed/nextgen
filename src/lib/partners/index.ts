...
export function getPartner<V extends Vertical>(v: V): ProviderMap[V] {
  switch (v) {
    ...
    case "payout":
      return (flags.payout && razorpayPayoutConfigured() ? razorpayPayout : mock.mockPayout) as ProviderMap[V];
    case "bbps":
...
export function partnerStatus() {
  return {
    ...
    payout:   { live: flags.payout && razorpayPayoutConfigured(), provider: flags.payout && razorpayPayoutConfigured() ? "RAZORPAYX" : "MOCK" },
